...
def get_continuous_rep(corpus, vocab=None, visualize='n'):
    """
    Generate continuous representations using word2vec embeddings.
    For each document, we average the embeddings of all token-head pairs.
    """
    document_vectors = []
...
def get_bestmatch(queries, keys, keys_df, metric=cosine_similarity):
    # Convert list of numpy arrays to a 2D array
...
def get_results(queries_df, keys_df, get_rep, use_cache=False):
    ...
    # Note: No need for vocabulary building with continuous embeddings
...
# Run with continuous embeddings
